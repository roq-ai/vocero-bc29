...
import { getClientInfos } from 'apiSdk/client-infos';
import { ClientInfoInterface } from 'interfaces/client-info';
import { getUserNotifications } from 'apiSdk/user-notifications';
...
function DashboardPage() {
  ...
  const clientInfoStat = useSWR<PaginatedInterface<ClientInfoInterface>>(
    () => `/client-infos`,
    () => getClientInfos({ limit: 1, offset: 0 }),
  );

  const userNotificationsStat = useSWR<PaginatedInterface<UserNotificationsInterface>>(
    () => `/user-notifications`,
    () => getUserNotifications({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Client Infos',
      result: clientInfoStat?.data?.totalCount,
      isLoading: clientInfoStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Notifications',
      result: userNotificationsStat?.data?.totalCount,
      isLoading: userNotificationsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
